...
function TextBox(str, fontFamily, fontSize, w, h, fontColor, bgColor, horizontalAlign, verticalAlign, offsetX, offsetY) {
	this.str = str;
    this.width = w;
    this.height = h;
...
TextBox.prototype.render = function(str, canvas) {
	...
	if(this.verticalAlign == "top") {
		...
		py = this.height;
	} else if(this.verticalAlign == "fixed") {
		py = 0.0;
	} else {
		// centered
		py =  h - bounds[3] + (this.height-h)/2;
	}

	px += this.offsetX;
	py += this.offsetY;

	canvas.drawText(paint, str, px, py);
}
